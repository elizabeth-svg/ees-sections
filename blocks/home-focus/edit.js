( function ( wp ) {
	...
	wp.blocks.registerBlockType( 'ees/home-focus', {
		edit: function ( props ) {
			...
			function addRow() {
				setRows( rows.concat( [ { title: '', desc: '', url: '' } ] ) );
			}
			...
			var rowEls = rows.map( function ( row, i ) {
				return el( 'div', { key: i, style: { border: '1px solid #2a2a30', borderRadius: '6px', padding: '12px', margin: '0 0 12px', background: '#0d0d10' } },
					el( 'div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' } },
						el( 'strong', { style: { color: '#f2ea36', fontFamily: 'monospace', fontSize: '11px' } }, pad2( i + 1 ) ),
						el( 'div', null,
					...
					el( TextControl, { label: 'Description', value: row.desc || '', onChange: function ( v ) { updateRow( i, 'desc', v ); } } ),
			...
			return el( 'div', { style: { padding: '20px', border: '1px dashed #b7b7b7', borderRadius: '8px', background: '#0d0d10', color: '#f7f7f5', fontFamily: 'system-ui' } },
				el( 'p', { style: { color: '#a8a8a4', fontFamily: 'monospace', fontSize: '11px', margin: '0 0 4px' } }, 'Home · Focus areas' ),
